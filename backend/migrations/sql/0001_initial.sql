...
-- ===========================================================================
-- Helper expression: current_setting('app.current_org', true)::uuid
-- Enable + FORCE RLS on tenant-owned tables; policy restricts to current org.
DO $$
DECLARE t text;
BEGIN
  FOREACH t IN ARRAY ARRAY[
    'app_user','oauth_identity','service_account','session','refresh_token',
    ...
    EXECUTE format($p$CREATE POLICY %1$s_tenant_isolation ON %1$I
                      ...
                      WITH CHECK (organization_id = current_setting('app.current_org', true)::uuid);$p$, t);
  END LOOP;
END $$;
